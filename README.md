...
| S3 | 실종자 상세 | 불필요 | 사진 캐러셀, 인상착의, 경로 미리보기 지도, **제보 타임라인** · 보호자면 **보호자 모드**(정보 수정 · 사진 추가 · 발견 완료 · 제보 확인/숨기기) |
...
현재 `flutter analyze` **이슈 0건**, `flutter test` **477개 통과**.
...
- **결과**: **477개 테스트**가 실제 네트워크 없이 약 35초에 돈다. 로직(인터셉터·예외 변환·EXIF·지도 계산)은 테스트를 먼저 쓰고 구현했다.
