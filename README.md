...
| S3 | 실종자 상세 | 불필요 | 사진 캐러셀, 인상착의, 경로 미리보기 지도, **제보 타임라인**(목격 일시 · 이전 지점에서 이동 방향·거리 · 1시간 넘는 공백), **카카오톡 공유** · 발견 완료된 사건은 제보 버튼 대신 "찾았어요" · 보호자면 **보호자 모드**(정보 수정 · 사진 추가 · 발견 완료 · 제보 확인/숨기기) |
...
| S6 | 로그인 (모달) | — | 카카오 로그인 |
| S7 | 실종자 등록 | **필요** | 사진 다중 선택 · 나이로 구분(아동·성인·어르신) 자동 선택 · 지도에서 위치 고르기 · 임시 저장 |
| S8 | MY | 분기 | 내가 등록한 실종자 · 발견 완료 처리 · 내 제보 이력(누르면 사건 상세) · 알림 설정(반경·대상·야간) · 로그아웃 |
| — | 알림함 | 불필요 | 탭 상단바 알림 버튼(안 읽음 점) → 받은 알림 최신순, 누르면 사건 상세 |
...
| **사용 방법** | 지도 탭 → 검색·필터로 사건을 고르면 실종 지점부터 **번호 핀이 폴리라인으로** 이어진다. **슬라이더를 끌면 그 시각까지의 경로만** 남고, "60% 이상만" 토글로 신뢰도 높은 제보만 볼 수 있다. 핀을 누르면 제보 사진·유사도가 시트로 열린다. 사건 상세 타임라인에는 제보마다 **"↗ 북동쪽 0.9km"** 처럼 앞 지점에서 어디로 움직였는지와, 1시간 넘게 아무도 못 본 **공백 구간**을 적는다 |
...
| **사용 방법** | 사건 상세 → **제보하기** → 사진(촬영/앨범) → 위치·시각이 **이미 채워져 있음** → 필요하면 "인하공전 도서관 앞" 같은 장소 이름만 덧붙이기 → 분석 → 제보 |
...
| 알림 | 받는 사람 | 누르면 |
|---|---|---|
| 🆕 새 실종 신고 | 반경 안 시민 (기기마다 1/3/5/10km·대상·야간 수신 설정, 등록자 본인 제외) | 사건 상세 |
| 👀 목격 제보 도착 | 그 사건의 보호자 (반경·방해 금지 시간 무시) | 사건 상세 |
| 🎉 발견 완료 | 그 사건에 제보한 시민 (게스트 포함) | 사건 상세 |

- **앱 구현**: `firebase_messaging`으로 FCM 토큰을 받아 `DeviceRegistrar`가 기기 해시·현재 좌표와 함께 `POST /devices`에 등록 (로그인 불필요, 토큰 갱신·로그인·알림 설정 변경 시 재등록해 보호자 알림이 계정의 기기를 찾고 반경·대상·야간 설정이 바로 반영되게 함). 안드로이드는 앱이 전면에 있을 때 시스템 알림을 그리지 않아 **앱 안에 알림 바(`PushAlertBar`)를 직접 띄운다.** 알림 `data.type`으로 이동할 화면을 정하고, **모르는 알림 종류면 화면을 열지 않는다.**
- Firebase 초기화에 실패해도 `SilentPushMessaging`으로 대체돼 **앱은 알림 없이 정상 실행**된다.
- **알림함**: 서버가 푸시를 보낼 때 받는 사람마다 기록해 두어(`GET /me/notifications`), 폰이 꺼져 있어 푸시를 놓쳤어도 상단바 알림 버튼에서 다시 찾을 수 있다. 안 읽은 알림이 있으면 버튼에 점을 찍고, 열면 전부 읽음이 되지만 보는 동안은 새로 온 줄을 강조한다. 앱이 앞으로 돌아오거나 푸시가 도착하면 다시 받는다.
- **공유와 링크로 열기**: 상세의 공유 버튼이 **카카오톡 피드 카드**(사진·이름·위치·경과 시간)를 보낸다. 받은 사람이 [곁애에서 보기]를 누르면 앱이 그 사건 상세로 열리고, 뒤로 대신 **작은 심볼**을 둬 누르면 홈으로 간다(외부 유입). 시연용 앱 링크 `gyeotae://missing/{id}`도 같은 흐름이다. 알림·링크로 앱을 **처음 켠 경우** 스플래시가 끝날 때까지 기다렸다가 연다.
...
| **사용 방법** | 홈 또는 실종자 탭에서 **실종자 등록** → 앨범에서 사진 여러 장 한 번에 선택 → **지도 가운데 핀을 옮겨** 마지막 위치 지정(주소 자동 표시) → 등록하면 새 사건 상세로 이동. 이후 **상세 화면이 보호자 모드**로 바뀌어 정보 수정·사진 추가·발견 완료·제보 확인/숨기기를 한 화면에서 한다 |
...
| **kakao_flutter_sdk_user · kakao_flutter_sdk_share** | 카카오 로그인 (카카오톡이 있으면 톡으로, 없으면 카카오계정으로), 사건 카카오톡 공유 |
| **app_links** | 앱 링크(`gyeotae://`)와 카카오톡 공유 링크를 받아 사건 상세 열기 |
...
| Kakao Talk Sharing (네이티브 앱 키) | 사건 상세를 카카오톡 피드 메시지로 공유, 버튼으로 앱 실행 |
...
| `POST` | `/devices` | 앱 시작·토큰 갱신·로그인·S8 알림 설정 변경 시 (푸시 등록) |
...
| `POST` | `/missing/{id}/resolve` | S3 보호자 · S8 발견 완료 |
| `PATCH` | `/missing/{id}` | S3 보호자 정보 수정 |
| `POST` | `/missing/{id}/photos` | S3 보호자 사진 추가 |
| `PATCH` | `/reports/{id}` | S3 보호자 제보 확인·숨기기 |
| `GET` | `/me/missing` · `/me/reports` | S8 MY |
| `GET` · `POST` | `/me/notifications` · `/me/notifications/read` | 탭 상단바 알림 점 · 알림함 |
...
│       └── notifications/     # 알림함 — 상단바 알림 버튼, 안 읽음 점
├── test/
...
현재 `flutter analyze` **이슈 0건**, `flutter test` **520개 통과**.
...
| 5 | B | 촬영 시각·위치가 다른 사진으로 2~3건 더 제보 | 사건 상세 **제보 타임라인**에 등급별로 쌓이고, 제보마다 이동 방향·거리가 붙음 |
| 6 | A/B | **지도 탭 → 사건 선택** → **시간 슬라이더** 끌기, "60% 이상만" 토글 | ⭐ 실종 지점부터 **경로가 시간순으로 자라남** |
| 7 | A | MY → **발견 완료** | B 기기에 **"찾았습니다"** 푸시, 목록 맨 아래에 흐리게 남고 상세의 제보 버튼이 "찾았어요"로 바뀜 |
...
- **결과**: **520개 테스트**가 실제 네트워크 없이 약 35초에 돈다. 로직(인터셉터·예외 변환·EXIF·지도 계산)은 테스트를 먼저 쓰고 구현했다.
...
| **제보 신고** | 보호자가 숨기기로 거른다 | 시민이 허위·장난 제보를 신고(F-3.5.14), 여러 번 신고된 제보는 자동 숨김 후 운영 검토 |
| **공유와 외부 유입** | 카카오톡 공유 → 앱이 있으면 상세로 열림 (Android) | 앱이 없는 사람도 볼 수 있는 **웹 상세 페이지**와 https 앱 링크, iOS 카카오 스킴 등록 |
