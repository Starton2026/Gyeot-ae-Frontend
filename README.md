...
| S8 | MY | 분기 | 내가 등록한 실종자 · 발견 완료 처리 · 내 제보 이력 · 알림 설정(반경·대상·야간) · 로그아웃 |
...
현재 `flutter analyze` **이슈 0건**, `flutter test` **447개 통과**.
...
- **결과**: **447개 테스트**가 실제 네트워크 없이 약 35초에 돈다. 로직(인터셉터·예외 변환·EXIF·지도 계산)은 테스트를 먼저 쓰고 구현했다.
